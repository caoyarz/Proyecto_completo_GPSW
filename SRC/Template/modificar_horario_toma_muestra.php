    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <div id="content">
        ...
        <div class="container-fluid">
          ...
          <form method="post">
                <p>Rut paciente: <input class="form-control" type="text" name="rut" id="rut"/></p>
                <center>
            ...
            <?php
                if(isset($_POST['buscar'])){
                    if($_POST['rut']==null){
                        echo '<script language="javascript">alert("No se ingresó ningún Rut");</script>';
                    }
                    else{
                        echo"Citas de el paciente <br>";
                        echo '<table class="egy">';
                        echo"<tr><th>hora cita</th></tr>";
                        $rut=$_POST['rut'];
                        $sql="select SGL_CITA_HORA,SGL_CITA_IDENTIFICADOR from SGL_CITA where SGL_PACIENTE_RUT='".$rut."' 
                              and SGL_CITA_IDENTIFICADOR not in (select SGL_CITA_IDENTIFICADOR from SGL_CITA 
                                                                 where SGL_PACIENTE_RUT='".$rut."' and SGL_CITA_ESTADO='Realizada')";
                        $result=mysqli_query($conn,$sql);
                        while($mostrar=mysqli_fetch_array($result)){
                            $cod=$mostrar['SGL_CITA_IDENTIFICADOR'];
                            echo "<tr>";
                            echo"<th> <a href='modificar_horario_toma_muestra2.php?var=$cod'>";
